  ...
  const render_list = techStack.map((item, index) => {
    return (
      <>
        <p
          key={index}
          className="text-beige-800 rounded-lg  p-1.5 bg-white shadow-md flex items-center mb-2 mx-auto"
        >
  ...
  return (
    <>
      <a
        ...
        className=" bg-beige-50 res-card border-2 border-gray-200 shadow-md p-4 m-2 transition duration-700 hover:border-beige-700 font-mono"
      >
        ...
        <div className="mt-5 flex flex-wrap justify-left">{render_list}</div>
      </a>
    </>
  );
}
